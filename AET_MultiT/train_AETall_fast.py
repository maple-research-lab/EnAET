...
def train_AETAllfast(train_dataloader,valid_dataloader, model, optimizer,
    ...
    print("one iter for labelled data %d, one iter for unlabeled data %d"%(labelled_reload,unlabelled_reload))
    for batch_idx in range(iteration):
        try:
            read_data=prefetcher_label.next()
            if (batch_idx%labelled_reload==0  and batch_idx!=0) or read_data is None:
                print("reloading labeled dataloader again!!")
                prefetcher_label = data_prefetcher(train_dataloader)
            (img1, img1_t), img2t, img3t, img4t, img5t, img6t, img7t,aff_parat, transform_matrix1t,oper_paramst, target = read_data
        except:
            #problems happen here, we need to reload the dataloader by our self
            print("reloading labeled dataloader again!!")##very important to check if the reloading worked in fast mode
            prefetcher_label = data_prefetcher(train_dataloader)
            (img1, img1_t), img2t, img3t, img4t, img5t, img6t, img7t,aff_parat, transform_matrix1t,oper_paramst, target = prefetcher_label.next()
        #img1 = read_data[0]  # original images
        #target = read_data[1]  # target label

        try:
            read_data1= prefetcher_unlabel.next()
            if (batch_idx%unlabelled_reload==0 and batch_idx!=0) or read_data1 is None:
                print("reloading unlabeled dataloader again!!")
                prefetcher_unlabel = data_prefetcher(valid_dataloader)
            (img1_u, img1_u2), img2, img3, img4, img5, img6, img7,aff_para, transform_matrix1,oper_params, _ =read_data1
        except:
            print("reloading unlabeled dataloader again!!")  ##very important to check if the reloading worked in fast mode
            prefetcher_unlabel = data_prefetcher(valid_dataloader)
            (img1_u, img1_u2), img2, img3, img4, img5, img6, img7,aff_para, transform_matrix1,oper_params, _ = prefetcher_unlabel.next()
        ...
        if use_aet:
            ...
            loss += aet_loss * min(lambda_aet * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0),
                                   params['max_lambda'])
            WAET1.update(
                min(lambda_aet * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0), params['max_lambda']),
                img1.size(0))
            if params['KL_Lambda']!=0:
                #calculate KL mode
                use_key1 = use_key + ['classifier']
            ...
            loss += aet_loss2 * min(params['lambda1'] * np.clip((epoch * iteration + batch_idx) /(1024*iteration), 0.0, 1.0),
                                    params['max_lambda1'])
            WAET2.update(
                min(params['lambda1'] * np.clip((epoch * iteration + batch_idx) /(1024*iteration), 0.0, 1.0),
                    params['max_lambda1']),
                img1.size(0))
            if params['KL_Lambda']!=0:
                #calculate KL mode
            ...
            loss += aet_loss3 * min(params['lambda2'] * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0),
                                    params['max_lambda2'])
            WAET3.update(
                min(params['lambda2'] * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0),
                    params['max_lambda2']),
                img1.size(0))
            if params['KL_Lambda']!=0:
                #calculate KL mode
            ...
            loss += aet_loss4 * min(params['lambda3'] * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0),
                                    params['max_lambda3'])
            WAET4.update(
                min(params['lambda3'] * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0),
                    params['max_lambda3']),
                img1.size(0))
            if params['KL_Lambda']!=0:
                #calculate KL mode
            ...
            loss += aet_loss5 * min(params['lambda4'] * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0),
                                    params['max_lambda4'])
            WAET5.update(
                min(params['lambda4'] * np.clip((epoch * iteration + batch_idx) / (1024*iteration), 0.0, 1.0),
                    params['max_lambda4']),
                img1.size(0))
